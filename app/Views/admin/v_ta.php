                ...
                <tbody>
                    ...
                    foreach ($ta as $key => $value) { ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><?= $value['tahun'] ?></td>
                            <td><?= $value['ta'] ?> <?= ($value['status'] == 1) ? '<label class="badge badge-success">Active</label>' : '' ?></td>
                            <td><?= ($value['pendaftaran'] == 1) ? '<label class="badge badge-success">Dibuka</label>' : '<label class="badge badge-danger">Ditutup</label>' ?></td>
                            <td>
                                <?php if ($value['pendaftaran'] == 0) { ?>
                                    <a href="<?= base_url('ta/pendaftaranBuka/' . $value['id_ta']) ?>" class="btn btn-primary btn-xs btn-flat">Buka</a>
                                <?php } else { ?>
                                    <a href="<?= base_url('ta/pendaftaranTutup/' . $value['id_ta']) ?>" class="btn btn-danger btn-xs btn-flat">Tutup</a>
                                <?php } ?>
                            </td>
                            <td>
...
<?php foreach ($ta as $key => $value) { ?>
    <div class="modal fade" id="delete<?= $value['id_ta'] ?>">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Hapus Tahun Jaran</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
